...
LATEST_SCRIPT_TAG=$(curl -s -u $GIT_USER:$GIT_TOKEN https://api.github.com/repos/lorenyeung/${GIT_REPO}/releases/latest | jq -r '.tag_name')
SCRIPT_DIR=$( cd -- "$( dirname -- "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )
test=$(curl -s -u $GIT_USER:$GIT_TOKEN https://api.github.com/repos/lorenyeung/${GIT_REPO}/releases/latest)
echo "test var $test"
